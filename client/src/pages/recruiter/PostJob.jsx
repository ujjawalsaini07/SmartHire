...
const PostJob = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (!verifyStatus.data?.isVerified) {
          toast.error("Your account must be verified to post jobs");
          // Optional: redirect or show blocking UI
        }
        setIsVerified(verifyStatus.data?.isVerified || false);

        // 2. Fetch Categories
        const cats = await publicApi.getAllCategories();
        setCategories(cats.data || []);

        // 3. Fetch Skills (initial batch)
        const sks = await publicApi.getAllSkills({ limit: 100 });
        setSkills(sks.data || []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setNewQualification('');
      setNewQuestion({ question: '', isRequired: false });
      setSkillSearch(''); // Clean up search too
      
      // navigate('/recruiter/jobs'); // User requested to stay on page
    } catch (error) {
  ...
  if (verifying) return <div className="min-h-screen flex items-center justify-center"><Spinner size="lg" /></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg py-8">
      <div className="container-custom max-w-4xl mx-auto">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-light-text dark:text-dark-text">Post a New Job</h1>
        ...
        {!isVerified && (
           <div className="bg-error-50 dark:bg-error-900/20 border border-error-200 dark:border-error-800 rounded-lg p-4 mb-6 flex items-start space-x-3">
             <AlertCircle className="w-5 h-5 text-error-600 dark:text-error-400 mt-0.5" />
             <div>
               <h3 className="font-semibold text-error-900 dark:text-error-100">Verification Required</h3>
               <p className="text-sm text-error-700 dark:text-error-300">
                 Your account needs to be verified before you can post public jobs. You can still save drafts.
               </p>
             </div>
           </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end space-x-4 pt-4">
             ...
             <Button 
               ...
               className="w-40"
             >
               {loading ? <Spinner size="sm" /> : <><Save className="w-4 h-4 mr-2" /> Post Job</>}
             </Button>
          </div>
