...
// import { Briefcase, Bookmark, FileText, ChevronRight, Clock, CheckCircle, Star, MapPin, X, Bell } from 'lucide-react';
...
import { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { Briefcase, Bookmark, FileText, ChevronRight, CheckCircle, Star, MapPin, X, Bell } from 'lucide-react';
import { jobSeekerApi } from '@api/jobSeekerApi';
import { notificationApi } from '@api/notificationApi';
import useAuthStore from '@store/authStore';
import Card from '@components/common/Card';
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStats({
        ...
        savedJobs:
          savedRes.count ||
          savedRes.data?.savedJobs?.length ||
          savedRes.savedJobs?.length ||
          0,
      });
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {recentApplications.map((app) => (
                <div
                  key={app._id}
                  className="flex items-center justify-between p-4 rounded-xl border border-gray-100 dark:border-dark-border bg-gray-50/50 dark:bg-dark-bg-secondary hover:shadow-sm transition-all"
                >
                  <div>
                    <h3 className="font-semibold text-gray-900 dark:text-white">
                      {app.jobId?.title || 'Unknown Role'}
