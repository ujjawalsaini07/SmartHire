...
JobViewSchema.statics.recordView = async function (jobId, userId = null, ipAddress = null, userAgent = null) {
  try {
...
JobViewSchema.statics.getViewsByDateRange = function (jobId, startDate, endDate) {
  return this.find({
    jobId,
    viewedAt: {
...
JobViewSchema.statics.getMostViewedJobs = async function (limit = 10, days = 30) {
  const startDate = new Date();
  startDate.setDate(startDate.getDate() - days);
...
JobViewSchema.statics.getViewTrends = async function (jobId) {
  ...
  const percentageChange = previousPeriod > 0 
    ? ((change / previousPeriod) * 100).toFixed(2) 
    : 0;
...
JobViewSchema.statics.cleanupOldAnonymousViews = async function (daysToKeep = 90) {
