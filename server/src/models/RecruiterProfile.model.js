...
RecruiterProfileSchema.methods.rejectVerification = async function (adminId, notes) {
...
RecruiterProfileSchema.methods.calculateProfileCompleteness = function () {
  ...
  if (this.location?.city && this.location?.country) completeness += weights.location;
...
RecruiterProfileSchema.pre("save", function () {
  ...
  if (
    (this.verificationStatus === "pending" || this.verificationStatus === "rejected") &&
    this.isVerified
  ) {
    this.isVerified = false;
  }
